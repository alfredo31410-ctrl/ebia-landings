...
function invalidRedirect(request: Request) {
  const response = NextResponse.redirect(new URL("/landings/ia-desde-cero?registro=confirmacion_invalida", request.url), 303);
...
export async function GET(request: Request) {
  ...
  const response = NextResponse.redirect(new URL("/landings/ia-desde-cero/gracias", request.url), 303);
  response.headers.set("X-Robots-Tag", "noindex, nofollow");
