...
import { buildCheckoutUrl, getValidCheckoutUrl } from "@/lib/checkout";
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const trackView = () => {
      ...
      if (typeof window.fbq === "function") { window.fbq("track", "ViewContent", { content_name: content.checkoutName }); viewed.current = true; return; }
  ...
  const buy = (event: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window.fbq === "function") window.fbq("trackCustom", "CheckoutButtonClick", { content_name: content.checkoutName, value: 1490, currency: content.currency });
  };
  const href = checkout ?? "#checkout-status";
  return <main className="ecosystem-page">
    <div className="ecosystem-topline"><span className="status-dot" /> Acceso digital · ruta guiada para empezar con IA</div>
    <header className="ecosystem-header"><div className="ecosystem-shell"><a className="ecosystem-brand" href="#inicio" aria-label="EBIA, inicio">EBIA<span>.</span></a><a className="mini-cta" href={href} onClick={buy}>Acceder al programa <Arrow /></a></div></header>

    <section className="ecosystem-hero" id="inicio"><div className="ecosystem-shell ecosystem-hero-grid"><div className="ecosystem-hero-copy"><p className="ecosystem-eyebrow">{content.eyebrow}</p><h1>{content.headline}<em>{content.accentHeadline}</em></h1><p className="ecosystem-lead">{content.subheadline}</p><div className="ecosystem-actions"><a className="ecosystem-primary" href={href} onClick={buy}>Quiero acceder al programa <Arrow /></a><a className="ecosystem-text-link" href="#incluye">Ver qué incluye <span>↓</span></a></div><p className="ecosystem-microcopy">{content.payment} · {content.price} {content.currency} · Checkout seguro con Hotmart</p></div><div className="ecosystem-visual" aria-label="Vista previa del sistema de aprendizaje" role="img"><div className="visual-glow" /><div className="visual-grid" /><div className="visual-card visual-card--main"><small>tu ruta de aplicación</small><strong>Idea → instrucción → resultado</strong><div className="visual-progress"><i /><i /><i /></div><span>Aprende a repetir lo que funciona</span></div><div className="visual-card visual-card--float"><b>IA</b><span>criterio<br />+ práctica</span></div><div className="visual-orbit orbit-one" /><div className="visual-orbit orbit-two" /></div></div></section>

    <section className="ecosystem-trust"><div className="ecosystem-shell trust-grid"><div><small>FORMATO</small><strong>{content.modality}</strong></div><div><small>NIVEL</small><strong>{content.level}</strong></div><div><small>ACCESO</small><strong>{content.duration}</strong></div><div><small>PROCESO</small><strong>Pago con Hotmart</strong></div></div></section>
    ...
    <section className="ecosystem-section includes-section" id="incluye"><div className="ecosystem-shell include-grid"><div><p className="ecosystem-kicker">Dentro del ecosistema</p><h2>Todo lo necesario para practicar sin empezar de cero.</h2><p className="muted">El contenido y las condiciones mostradas son provisionales y deben sustituirse con la oferta final.</p><a className="ecosystem-primary" href={href} onClick={buy}>Quiero empezar a aplicar <Arrow /></a></div><div className="include-list"><div><span>01</span><p><strong>Ruta de aprendizaje</strong><br />Cuatro módulos para pasar de conceptos a aplicación.</p></div><div><span>02</span><p><strong>Biblioteca de plantillas</strong><br />Puntos de partida para practicar con más estructura.</p></div><div><span>03</span><p><strong>Checklist de revisión</strong><br />Una guía para evaluar antes de usar un resultado.</p></div><div><span>04</span><p><strong>{content.support}</strong><br />Este dato requiere confirmación antes de publicar.</p></div></div></div></section>
    ...
    <section className="ecosystem-section offer-section"><div className="ecosystem-shell offer-grid"><div><p className="ecosystem-kicker">Tu acceso incluye</p><h2>Una base para trabajar con IA con más criterio.</h2><ul><li>Programa práctico de 4 módulos</li><li>Biblioteca de plantillas</li><li>Checklist de revisión</li><li>{content.duration}</li></ul></div><aside className="price-card" id="checkout"><p className="ecosystem-kicker">{content.product}</p><div className="price">{content.price}<small> {content.currency}</small></div><p>{content.payment}</p><a className="ecosystem-primary" href={href} onClick={buy}>Quiero acceder al programa <Arrow /></a><small className="secure-note">Procesamiento seguro con Hotmart.<br />{content.guarantee}</small><p id="checkout-status" tabIndex={-1} className={!checkout ? "checkout-warning" : "checkout-warning is-hidden"}>El checkout todavía no está configurado. Añade NEXT_PUBLIC_HOTMART_CHECKOUT_URL para activar los botones.</p></aside></div></section>
    ...
    <section className="ecosystem-final"><div className="ecosystem-shell"><p className="ecosystem-kicker">Empieza con una ruta</p><h2>Menos ruido. Más criterio para aplicar IA.</h2><p>{content.subheadline}</p><div className="final-price">{content.price} <small>{content.currency} · {content.payment}</small></div><a className="ecosystem-primary" href={href} onClick={buy}>Quiero acceder al programa <Arrow /></a><small>Checkout seguro con Hotmart · Los resultados dependen de tu aplicación.</small></div></section>
    <footer className="ecosystem-footer"><div className="ecosystem-shell"><strong>EBIA<span>.</span></strong><p>Formación digital práctica. Datos de oferta provisionales, sujetos a confirmación.</p><small>© 2026 EBIA · Privacidad · Términos · Soporte</small></div></footer>
  </main>;
}
