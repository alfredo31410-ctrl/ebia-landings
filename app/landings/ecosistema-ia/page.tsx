...
export default function Page() {
  ...
  const cta = "INSCRIBIRME AHORA AL RETO";
  return <main className="ecosystem-page challenge-page">
    ...
    <section className="ecosystem-section offer-section"><div className="ecosystem-shell offer-grid"><div><p className="ecosystem-kicker">Tu acceso</p><h2>Inteligencia artificial aplicada desde cero.</h2><p className="offer-description">Acceso al Reto Práctico de Inteligencia Artificial de 3 Días.</p></div><aside className="price-card" id="checkout"><p className="ecosystem-kicker">{content.product.shortName}</p><div className="price">{content.price.formatted}</div><p>{content.price.paymentLabel}</p><a className="ecosystem-primary" href={href} onClick={buy}>{cta} <Arrow /></a><small className="secure-note">Checkout seguro con Hotmart.<br />La compra es procesada para {content.seller}.</small>{!checkout && <p id="checkout-status" tabIndex={-1} className="checkout-warning">No fue posible preparar el checkout. Inténtalo de nuevo más tarde.</p>}</aside></div></section>
