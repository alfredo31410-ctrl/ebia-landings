...
const IA_NONCE_ENDPOINT = "/landings/ia-desde-cero/api/registrations/nonce";
...
export async function requestRegistrationNonce(landing: string): Promise<RegistrationNonceResponse> {
  ...
  const response = await fetch(IA_NONCE_ENDPOINT, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ landing }), cache: "no-store" });
